...
exports.addressToScript = function (addr) {
    ...
    var decoded = Buffer.from(base58.decode(addr));
    ...
    if (payload.length === 23) {
        pubkey = payload.slice(3);
    } else if (payload.length === 21) {
        pubkey = payload.slice(1);
    } else {
        console.error('invalid address payload length for ' + addr);
        throw new Error();
    }

    if (pubkey.length !== 20) {
        console.error('invalid pubkey hash length for ' + addr);
        throw new Error();
    }
